"""Make Hawks Anakin: his look, his class and his lightsaber, by assignment.

One of three pieces. This is the half that has never failed - every edit in
it repoints a soft object reference, which is the one shape this game
reliably accepts. Run anakin_unlock.py as well to make his kit selectable
rather than only worn.

They build separate containers, so either can be installed without the
other. Worth knowing that two containers have not always both taken effect
here, so if something goes missing, try them one at a time.

    python examples/anakin_look.py
...
ASSIGN = {
    ...
    # Not CPD_TacticalSpec_Anakin. His own spec names no slot in AllowedSlots,
    # so the picker never lists it, and an equipped part that is not in the list
    # draws as "None". anakin_unlock turns CPD_TacticalSpec_Padawan_SlugHQ into
    # the Jedi Knight, and that one the picker does list, so equipping it shows
    # a name and an icon and can be changed and changed back.
    #
    # Without anakin_unlock installed this is just Tel Rea's story-stage
    # Padawan, which is a reasonable thing for him to be wearing anyway.
